...
1. **Narrow `_TOKEN_RE`** from `[A-Za-z0-9+/_-]{24,}` to `[A-Za-z0-9+]{24,}` — drop `/`, `-`, **and `_`** from the class. Paths, URLs, branches, hyphenated slugs, and snake_case/dunder identifiers cannot form a candidate token *by construction*; the entropy net only ever judges contiguous unstructured runs (base64/hex/API-key shapes). *(Amended during implementation: the corpus replay showed underscore identifiers — `mcp__plugin_*` tool names, Python function names — were 77% of the residual false positives; `_` is a word separator in identifiers exactly like `-`. Known underscore vendor shapes — `github_pat_*`, `gh*_*`, `sk-proj-*` — remain named-pattern-covered, and the replay's genuinely token-shaped survivors were all caught without `_` in the class.)* `+` stays: base64 material, never a word separator.
